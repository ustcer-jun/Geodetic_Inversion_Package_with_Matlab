...
line([SF, SF], ylim, 'Color', 'k', 'LineStyle', '-', 'LineWidth', 2);
legend('Phase','Phase grd',['SF = ',num2str(SF)],'Location','best',fontsize=12);
% line([SF,SF],ylim,'k','linestyle','-','linewidth',2);
% semilogx(SF0*exp(([1:N]-round(N/2))/2),pct0(:,3),'m-x')
% semilogx(SF0*exp(([1:N]-round(N/2))/2),pct0(:,4),'g-x')
% semilogx(SF0*exp(([1:N]-round(N/2))/2),pct0(:,5),'c-x')
% legend('Phase','Phase grd',['SF = ',num2str(SF)],'Azimuth','Gps','Opitical','Location','best',fontsize=12);
title(['Loop SF values ','(PGW = ',num2str(PGW),' )'],fontsize=16);
