...
% set the imput config file name under the model path
    config_inv = 'config.inv'; %% inversion and fault geometry configuration file。

% adding search path for the inversion package
    addpath '/Users/junye/MATLAB/Geodetic_Inversion/src'
...
% initiate the model
    ...
    xgrd=[]; ygrd=[]; dGrd=[]; tp_grd=[]; look_grd=[]; sGrd=[]; % phase gradient data
...
%% modify the config.inv file under model path to change the parameters
    ...
    xo = cell2mat(inifile(config_inv,'read',{'model','origin','xo','d'}));% the position of source
    ...
    PSC = cell2mat(inifile(config_inv,'read',{'inversion','inv_params','positivity_strike','d'}));% positive slip constraints
    ...
    PGW = cell2mat(inifile(config_inv,'read',{'inversion','inv_params','weight_ph_grd','d'}));
...
%% load the data and plot them
    ...
    % load the descending grdient data
    num_x_grd = cell2mat(inifile(config_inv,'read',{'data','data_params','num_x_grd_sources','d'}));
    if num_x_grd ~= 0
        for j = 1:1:num_x_grd
            index={'dec','asc'};
            dat = load ([input_path cell2mat(inifile(config_inv,'read',{'data','data_files',[index{j} '_grdx'],'s'}))]);
            [xx,yy] = utm2ll(dat(:,1)-X0+3,dat(:,2),0,1);
            dat(:,7) = dat(:,7);    
            %xx = xx - xo;
            yy = yy - yo; 
            xgrd = [xgrd;xx];
            ygrd = [ygrd;yy];
            dGrd = [dGrd;dat(:,7)];             % phase gradient 
            if TP~=0 tp_grd = [tp_grd;dat(:,3)]; else tp_grd = [tp_grd;dat(:,3)*0]; end      % topography (in m)
            % sGrd = [sGrd;dat(:,8)];             % error
            sGrd = [sGrd;ones(size(dat,1),1)]; 
        	look_grd = [look_grd;dat(:,4:6)];       % look vector (direction cosines)
            dat_ph_grd = [dat_ph_grd length(xx)];        
            plot_data(xx,yy,dat(:,7));     
            plot_trace(patch);
            title(['sampled LOS x grdient data  ',index{j}])
            ylabel('South - North',fontsize=14);
            xlabel('West - East',fontsize=14);
        end
    end
    % load the ascending grdient data
    num_y_grd = cell2mat(inifile(config_inv,'read',{'data','data_params','num_y_grd_sources','d'}));
    if num_y_grd ~= 0
        for j = 1:1:num_y_grd
            index={'dec','asc'};
            dat = load ([input_path cell2mat(inifile(config_inv,'read',{'data','data_files',[index{j} '_grdy'],'s'}))]);
            [xx,yy] = utm2ll(dat(:,1)-X0+3,dat(:,2),0,1);
            dat(:,7) = dat(:,7);     
            %xx = xx - xo;
            yy = yy - yo;
            xgrd = [xgrd;xx];             % longitude
            ygrd = [ygrd;yy];             % latitude
            dGrd = [dGrd;dat(:,7)];            
            if TP~=0 tp_grd = [tp_grd;dat(:,3)]; else tp_grd = [tp_grd;dat(:,3)*0]; end     % topography (in m)
            % sGrd = [sGrd;dat(:,8)];  
            sGrd = [sGrd;ones(size(dat,1),1)];      % error
            look_grd = [look_grd;dat(:,4:6)];       % look vector 
            dat_ph_grd = [dat_ph_grd length(xx)];  
            plot_data(xx,yy,dat(:,7));
            plot_trace(patch);
            title(['sampled LOS y grdient data  ',index{j}])
            ylabel('South - North',fontsize=14);
            xlabel('West - East',fontsize=14);
        end
    end
    ...
    if num_gps ~= 0
        ...
        for j = 1:1:num_gps
            ...
            if gps_h(j) ~= 0
                ...
                dG = [dG;[dat(:,3);dat(:,4)]/10];   % displacement in East North Up directions (in cm) 
                sG = [sG;[dat(:,6);dat(:,7)]/10];	% error in East North Up directions
...
%% Gnenrate the green's function
    GreenP = []; GreenA = []; GreenG = []; GreenO = [];GreenP_grd = [];
    ...
    if num_x_grd+num_y_grd ~= 0
        GreenP_grd = generate_greenp_grd(XS,YS,ZS,LL,WW,DIP,STRIKE,xgrd,ygrd,tp_grd,look_grd,nu,fault_type,dat_ph_grd,50,50); % here the 50 means the interval for gradient calculation
    end
    if num_azi ~= 0
        GreenA = generate_green_a(XS,YS,ZS,LL,WW,DIP,STRIKE,xA,yA,tpA,phi,nu,fault_type,dat_az);
    end
    if num_gps ~= 0
        GreenG = generate_green_g(XS,YS,ZS,LL,WW,DIP,STRIKE,xG,yG,tpG,nu,fault_type,[1,1],dat_gps);
    end
    ...
    Greens = [GreenP;GreenP_grd;GreenA;GreenG;GreenO];
    
%% create the smoothness matrix
    Smooth = [];
    if SF ~= 0
        %load Smooth
        Smooth = generate_smoothness(XS,YS,ZS,LL,WW,STRIKE,num_grid,fault_type,SDF,SSEG,SID,SSID,TOP,BOT,SIDE,SIDEID,1,2);
    end
...
%% add ramp to the dataset
    ...
    if RMP == 1
        ...
        rmp = generate_green_ramp(xP,yP,xA,yA,xgrd,ygrd,dat_ph,dat_ph_grd,dat_az,dat_gps*2,dat_gov,100);  
...
%%% prepare the observation vector for inversion
    ...
    if num_x_grd+num_y_grd ~= 0 && PGW ~= 0
        nGrd = mean(sGrd);
        sGrd = sGrd/mean(sGrd);%归一化？
        sGrd = sGrd/PGW*length(sGrd);
        D = [D; SWP*dGrd];
        %W = [W; ones(size(xgrd,1),1)];
        W = [W; 1./sGrd];
        
    elseif num_x_grd+num_y_grd ~= 0 && PGW == 0
        D = [D; SWP*dGrd];
        W = [W; sGrd*0];    
    end
    if num_azi ~= 0 && AW ~= 0
...
%%% set lsqlin params and bounds
    ...
    if RMP == 1
        ...
        rmp = generate_green_ramp(xP,yP,xA,yA,xgrd,ygrd,dat_ph,dat_ph_grd,dat_az,dat_gps,dat_gov,1e6);
        Green_all = [Greens rmp; Smooth/mean(max(Smooth,[],2))*SF/size(Smooth,1) zeros(size(Smooth,1),size(rmp,2))];
        for j = 1:1:length(W)
            Green_all(j,:) = Green_all(j,:)*W(j);
        end
    ...
    Us = U(1:2:end-size(rmp,2));    % strike slip
    Ud = U(2:2:end-size(rmp,2));    % dip slip
...
%%% plot the residuals and results
    ...
    num_dat = [0;sum(dat_ph); sum(dat_ph_grd);sum(dat_az); sum(dat_gps); sum(dat_gov)];
    ...
    fprintf(' reduction percentage for each dataset = %f%% %d%% %d%% %d%% %d%%, only %d%% to go!\n',pct(1),round(pct(2)),...
        round(pct(3)),round(pct(4)),round(pct(5)),500-round(sum(pct)));
...
%}    
    ...
    sn = (sign(ub(1:2:length(Us)*2))-0.5)*2;
    m = 2/3*log10(mu*(sqrt(sum(Us.*sn/100.*LL.*WW)^2+sum(Ud/100.*LL.*WW)^2)))-6.07;
    ...
    plot_res(Greens,rmp,U,D,Urmp0,rmp0,dat_ph,dat_ph_grd,dat_az,dat_gps,dat_gov,xP,yP,xgrd,ygrd,xA,yA,xG,yG,xO,yO,0,11,xo,yo)
    % forward=[Greens rmp]*U+(rmp0*Urmp0);
    % figure()
    % plot_data(xP,yP,forward);
    % xlabel('data points',fontsize=16);
    % ylabel('forward',fontsize=16);
