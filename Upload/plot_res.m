function plot_res(Greens,rmp,U,D,Urmp0,rmp0,dat_ph,dat_ph_grd,dat_az,dat_gps,dat_gov,xP,yP,xgrd,ygrd,xA,yA,xG,yG,xO,yO,output,zone,xo,yo)
    ...
    x = [xP;xgrd;xA;xG;xO];
    y = [yP;ygrd;yA;yG;yO];
    num_dat = [0 dat_ph dat_ph_grd dat_az dat_gps*3 dat_gov];
    for j = 1:1:length(dat_ph)+length(dat_ph_grd)+length(dat_az)+length(dat_gps)+length(dat_gov)
        if j <= length(dat_ph)+length(dat_ph_grd)+length(dat_az) || j > length(dat_ph)+length(dat_az)+length(dat_gps)+length(dat_ph_grd)
            if num_dat(j+1)~=0
